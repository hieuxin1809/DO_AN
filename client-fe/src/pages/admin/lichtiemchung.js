...
const pgCSS = `
.pg-lich{display:flex;gap:6px;list-style:none;padding:0;margin:0;flex-wrap:wrap}
.pg-lich li a{display:flex;align-items:center;justify-content:center;min-width:34px;height:34px;padding:0 10px;border-radius:8px;border:1.5px solid #e2e8f0;color:#64748b;font-size:13px;font-weight:600;cursor:pointer;text-decoration:none;transition:all .15s}
.pg-lich li a:hover{border-color:#2A388F;color:#2A388F;background:#eff6ff}
.pg-lich li.active a{background:linear-gradient(135deg,#2A388F,#0ea5e9);border-color:#2A388F;color:#fff}
.pg-lich li.disabled a{opacity:.4;cursor:not-allowed}
`;
...
const AdminLichTiemChung = () => {
  ...
  const handleDelete = async (id) => {
    const { isConfirmed } = await Swal.fire({
      title: 'Xóa lịch tiêm?',
      html: 'Bạn có chắc muốn xóa lịch tiêm này?',
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ background: '#fff', borderRadius: 14, padding: '14px 18px', marginBottom: 18,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 13, color: T2, whiteSpace: 'nowrap' }}>Từ:</span>
          <input type="date" value={from} onChange={e => setFrom(e.target.value)}
            style={{ border: `1.5px solid ${B}`, borderRadius: 9, padding: '7px 10px',
              fontSize: 13.5, color: T, outline: 'none' }} />
        </div>
        {/* to */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 13, color: T2, whiteSpace: 'nowrap' }}>Đến:</span>
              ...
              {loading ? (
                ...
                return (
                  <tr key={item.id}
                    ...
                    <td style={{ padding: '12px 14px', color: T2, fontWeight: 600, fontSize: 13 }}>
                      {curPage * PAGE_SIZE + idx + 1}
                    </td>
                    <td style={{ padding: '12px 14px' }}>
                      <span style={{ fontWeight: 700, color: T, fontSize: 13.5 }}>{item.vaccine?.name || '—'}</span>
                    </td>
                    <td style={{ padding: '12px 14px' }}>
                      <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 600,
                        background: `rgba(14,165,233,.08)`, color: A }}>
                    ...
                    <td style={{ padding: '12px 14px' }}>
                      <span style={{ fontWeight: 800, color: P }}>{item.limitPeople}</span>
                    </td>
                    <td style={{ padding: '12px 14px' }}>
                    ...
                    <td style={{ padding: '12px 14px' }}>
                      <div style={{ display: 'flex', gap: 5 }}>
                        <a href={`add-lich-tiem-chung?id=${item.id}`} title="Sửa" style={{
                          width: 32, height: 32, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center',
                          border: `1.5px solid ${W}22`, background: `${W}11`, color: W, textDecoration: 'none', fontSize: 13,
                          transition: 'all .15s',
                        }}
