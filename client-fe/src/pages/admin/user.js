...
async function handleAddAccount(event) {
  ...
  const { password, repassword, fullname, email, phone } =
...
const AdminUser = () => {
  ...
  async function handleRoleChange(userId, newRole) {
    const res = await fetch(
      `http://localhost:8080/api/user/admin/change-role?id=${userId}&role=${newRole}`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      setItems((prev) =>
        prev.map((item) =>
          item.id === userId
            ? {
                ...item,
                authorities: { ...item.authorities, name: newRole },
              }
            : item,
        ),
      );
  ...
  async function handleDeleteUser(id) {
    ...
    const res = await fetch(
      `http://localhost:8080/api/user/admin/delete?id=${id}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async function handleEditUser(event) {
    ...
    const res = await fetch(
      "http://localhost:8080/api/user/admin/all/update-infor",
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (res.ok) {
      ...
      const updatedItems = items.map((item) =>
        item.id === editUser.id ? { ...item, ...payload } : item,
      );
      setItems(updatedItems);
      setEditUser(null);
  ...
  async function lockOrUnlock(id, type) {
    ...
    const response = await fetch(
      `http://localhost:8080/api/user/admin/lockOrUnlockUser?id=${id}`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (response.ok) {
      toast.success(type === 1 ? "Mở khóa thành công" : "Khóa thành công");
      const updatedItems = items.map((item) =>
        item.id === id ? { ...item, actived: !item.actived } : item,
      );
      setItems(updatedItems);
    } else {
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <select
                      value={item.authorities.name}
                      onChange={async (e) => {
                        const newRole = e.target.value;
                        const oldRole = item.authorities.name;
                        ...
                        const res = await fetch(
                          `http://localhost:8080/api/user/admin/change-role?id=${item.id}&role=${newRole}`,
                          {
                            method: "POST",
                            headers: {
                              Authorization: `Bearer ${token}`,
                  ...
                  <td className="sticky-col">
                    <button
                      onClick={() =>
                        lockOrUnlock(item.id, item.actived ? 0 : 1)
                      }
                      className={
                        item.actived ? "btn btn-primary" : "btn btn-danger"
                      }
                    >
                      <i
                        className={item.actived ? "fa fa-lock" : "fa fa-unlock"}
                      ></i>{" "}
